...
GET_KEYS_URL = "https://login.tailscale.com/admin/api/keys"
LOGIN_ENTRY_URL = "https://login.tailscale.com/login"
...
class AutoLogin:
    ...
    def __init__(self, config):
        self.host = urlparse(GET_KEYS_URL).netloc
        ...
        # tc_local 处理类型安全
        tc_local_val = config.get('tc_local', '')
        if isinstance(tc_local_val, str):
            self.tc_local = tc_local_val.strip()
        else:
            # storage_state 本身是 dict，无需 strip
            self.tc_local = tc_local_val
        
        self.tc_proxy = config.get('cc_proxy', '').strip() if isinstance(config.get('cc_proxy', ''), str) else config.get('cc_proxy')
        self.proxy_url=test_proxy(self.tc_proxy)
        if not self.proxy_url:
            self.tc_proxy = config.get('wz_proxy')
    ...
    def get_clawcloud_cookies(self):
        ...
        """
        if not isinstance(self.tc_local, dict):
            return []
    
        cookies = self.tc_local.get("cookies", [])
    ...
    def get_local_storage_by_origin(self):
        ...
        """
        if not isinstance(self.tc_local, dict):
            # 增加容错：如果 tc_local 是 JSON 字符串则解析
            if isinstance(self.tc_local, str) and self.tc_local.strip().startswith('{'):
                try:
                    self.tc_local = json.loads(self.tc_local)
                except:
                    return []
            else:
                return []
    
        origins = self.tc_local.get("origins", [])
    ...
    def get_balance_with_token(self):
        ...
        if self.proxy_url:
            ...
            self.log(f"启用代理: {self.tc_proxy['server'][:-3]}***")
    ...
    def save_cookie(self, value):
        ...
        if self.secret.update('GH_SESSION', value):
            ...
            self.notify.send(title="tailscale 自动更新keys",content="🔑 <b>Cookie 已自动更新</b>\n\nGH_SESSION 已保存")
        else:
            # 通过 Telegram 发送
            self.notify.send(title="tailscale 自动更新keys",content=f"""🔑 <b>新 Cookie</b>
...
""")
    ...
    def wait_device(self, page):
        ...
        self.notify.send(title="tailscale 自动更新keys",content=f"""⚠️ <b>需要设备验证</b>
...
2️⃣ 或在 GitHub App 批准""")
        ...
        if self.shots:
            self.notify.send(title="tailscale 自动更新keys",content="设备验证页面",image_path=self.shots[-1])
        ...
        for i in range(DEVICE_VERIFY_WAIT):
            ...
            if i % 5 == 0:
                ...
                if 'verified-device' not in url and 'device-verification' not in url:
                    ...
                    self.notify.send(title="tailscale 自动更新keys",content="✅ <b>设备验证通过</b>")
                    return True
                try:
        ...
        self.notify.send(title="tailscale 自动更新keys",content="❌ <b>设备验证超时</b>")
        return False
    ...
    def wait_two_factor_mobile(self, page):
        ...
        self.notify.send(title="tailscale 自动更新keys",content=f"""⚠️ <b>需要两步验证（GitHub Mobile）</b>

请打开手机 GitHub App 批准本次登录（会让你确认一个数字）。
等待时间：{TWO_FACTOR_WAIT} 秒""")
        if shot:
            self.notify.send(title="tailscale 自动更新keys",content="两步验证页面（数字在图里）",mage_path=shot)
        ...
        for i in range(TWO_FACTOR_WAIT):
            ...
            if "github.com/sessions/two-factor/" not in url:
                ...
                self.notify.send(title="tailscale 自动更新keys",content="✅ <b>两步验证通过</b>")
                return True
            ...
            if i % 10 == 0 and i != 0:
                ...
                if shot:
                    self.notify.send(title="tailscale 自动更新keys",content=f"两步验证页面（第{i}秒）",image_path=shot)
        ...
        self.notify.send(title="tailscale 自动更新keys",content="❌ <b>两步验证超时</b>")
        return False
    ...
    def handle_2fa_code_input(self, page):
        ...
        # 发送提示并等待验证码
        self.notify.send(title="tailscale 自动更新keys",content=f"""🔐 <b>需要验证码登录</b>
...
等待时间：{TWO_FACTOR_WAIT} 秒""")
        if shot:
            self.notify.send(title="tailscale 自动更新keys",content="两步验证页面",image_path=shot)
        ...
        if not code:
            ...
            self.notify.send(title="tailscale 自动更新keys",content="❌ <b>等待验证码超时</b>")
            return False

        # 不打印验证码明文，只提示收到
        self.log("收到验证码，正在填入...", "SUCCESS")
        self.notify.send(title="tailscale 自动更新keys",content="✅ 收到验证码，正在填入...")
            ...
            try:
                ...
                if el.is_visible(timeout=2000):
                    ...
                    if "github.com/sessions/two-factor/" not in page.url:
                        ...
                        self.notify.send(title="tailscale 自动更新keys",content="✅ <b>验证码验证通过</b>")
                        return True
                    else:
                        self.log("验证码可能错误", "ERROR")
                        self.notify.send(title="tailscale 自动更新keys",content="❌ <b>验证码可能错误，请检查后重试</b>")
                        return False
            except:
                pass

        self.log("没找到验证码输入框", "ERROR")
        self.notify.send(title="tailscale 自动更新keys",content="❌ <b>没找到验证码输入框</b>")
        return False
    ...
    def check_and_process_domain(self, domain):
        ...
        # 检查是否为 signin 页面
        if domain.endswith('tailscale.com/login'):
            return "signin"
        ...
        # 检查是否是正常已登录的区域域名
        if domain.endswith('admin/machines'):
            return "logged"
    ...
    def run(self):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                for i in range(10):
                    try:
                        ...
                        if resault=="signin":
                            ...
                            if self.click(page, "GitHub 登录按钮"):
                                ...
                                try:
                                    ...
                                    self.log(f"⚠️ {page.url}未检测到 GitHub 跳转，等待3秒", "WARN")
                                    page.wait_for_timeout(3000)
                            if not self.click(page, desc="GitHub 登录按钮"):
                                shot = self.shot(page, "找不到 GitHub 按钮")
                                if shot:
                                    self.notify.send(title="tailscale 自动更新keys",content="找不到 GitHub 按钮",image_path=shot)
                                ...
                                for j in range(10):
                                    ...
                                    if resault=="redirect":
                                        ...
                                        try:
                                            page.wait_for_url("https://*.tailscale.com/admin/machines", timeout=60000)
                        ...
                        if i <10:
                            ...
                            browser.close()
                            return False,  None, f"访问 {GET_KEYS_URL} 失败！"   
                ...
                print("="*50 + "\n")
                if self.shots:
                    self.notify.send(title="tailscale 自动更新keys",content=f"✅ {self.gh_username}成功！",image_path=self.shots[-1])
            except Exception as e:
                ...
                traceback.print_exc()
                if self.shots:
                    self.notify.send(title="tailscale 自动更新keys",content=f"❌ {self.gh_username}:{str(e)}",image_path=self.shots[-1])
...
def main():
    ...
    # 初始化 SecretUpdater，会自动根据当前仓库用户名获取 token
    secret = SecretUpdater("TAILSCALE_AUTH_KEY", config_reader=config)
    # 读取
    tc_locals = secret.load() 
    

    if not accounts:
        print("❌ 错误: 未配置 TAILSCALE_ACCOUNTS")
        return
    if not proxies:
    ...
    for account, proxy  in zip(accounts, proxies):
        ...
        tc_info={}
        tc_info['gh_username'] = username
        #tc_info['gh_password'] = account.get('password')
        tc_info['cc_proxy'] = proxy
        tc_info['notify'] = notify
        tc_info['wz_proxy'] = proxies[-1]
        ...
        if isinstance(tc_locals, dict):
            # 预加载localStorage数据，验证有效不再使用 gh_session
            storage_state = None
            tc_local=tc_locals.get(username,'')
            if tc_local:
                try:
                    tc_info['tc_local'] =  json.loads(base64.b64decode(tc_local).decode("utf-8"))
                    print("✅ 已加载 storage_state")
                except Exception as e:
                    print(f"❌ 加载 storage_state 失败: {e}")
        else:
            print(f"⚠️ tc_locals 格式错误！{tc_locals}")
            tc_locals={}
            tc_info['tc_local'] = []

        try:

            auto_login= AutoLogin(tc_info)
    ...
    print(f"tc_locals数据大小: {len(json.dumps(tc_locals)) / 1024:.2f} KB")
    secret.update(tc_locals)
    # 发送结果
    notify.send(
        title="tailscale 自动更新keys汇总",
        content="\n".join(results)
    )
