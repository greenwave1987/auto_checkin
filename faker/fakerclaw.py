...
class AutoLogin:
    ...
    def __init__(self, config):
        ...
        # fk_local 处理类型安全
        fk_local_val = config.get('fk_local', '')
        if isinstance(fk_local_val, str):
            self.fk_local = fk_local_val.strip()
        else:
    ...
    def get_fakerclaw_cookies(self):
        ...
        cookies = self.fk_local.get("cookies", [])
    ...
    def get_local_storage_by_origin(self):
        ...
        """
        if not isinstance(self.fk_local, dict):
            # 增加容错：如果 fk_local 是 JSON 字符串则解析
            if isinstance(self.fk_local, str) and self.fk_local.strip().startswith('{'):
                try:
                    self.fk_local = json.loads(self.fk_local)
                except:
                    return []
            else:
                return []
...
def main():
    ...
    # 初始化 SecretUpdater，会自动根据当前仓库用户名获取 token
    secret = SecretUpdater("FAKERCLAW_LOCALS", config_reader=config)
    # 读取
    fk_locals = secret.load() 
    ...
    for account, proxy  in zip(accounts, proxies):
        ...
        if isinstance(fk_locals, dict):
            # 预加载localStorage数据，验证有效不再使用 gh_session
            storage_state = None
            fk_local=fk_locals.get(username,'')
            if fk_local:
                try:
                    fk_info['fk_local'] =  json.loads(base64.b64decode(fk_local).decode("utf-8"))
                    print("✅ 已加载 storage_state")
                except Exception as e:
                    print(f"❌ 加载 storage_state 失败: {e}")
        else:
            print(f"⚠️ fk_locals 格式错误！{fk_locals}")
            fk_locals={}
            fk_info['fk_local'] = []

        try:

            auto_login= AutoLogin(fk_info)
